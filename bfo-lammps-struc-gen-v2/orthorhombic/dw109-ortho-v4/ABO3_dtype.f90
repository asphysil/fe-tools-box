...
! REAL(dp_real), PARAMETER :: charge_A=1.38177, charge_B=0.99997, charge_O=-0.79391 ! A=Pb, B=Ti, O=O PbTiO3 compound
 INTEGER(dp_int), PARAMETER :: nx=2,ny=2,nz=10 ! cell dimension
 ...
 REAL(dp_real):: a11, a22, a33  ! a=b=c=3.86 PbTiO3 latice parameters

REAL(dp_real) :: a_opt_bfo=4.0 !3.8914675  !7.782935000/2.0
...
o1_atms, o2_atms, o3_atms, o_atms
...
INTEGER(dp_int), dimension(:,:),ALLOCATABLE :: bo6_neigh
...
public :: supercell_cubic, neighbour_atm1_atm2,neighbour_axis,&
         projection_car_axis, projection_car_xy1, projection_car_xy2, &
         dm_Zint_angle, dm_int_angle, &
         rotation_about_u,ab_1st_neighbours,rotation_about_z

CONTAINS

SUBROUTINE rotation_about_z(alpha,rot_mat)
  IMPLICIT NONE 
  REAL (dp_real), INTENT(IN) :: alpha
  REAL (dp_real), INTENT(OUT) :: rot_mat(3,3)
  
  REAL (dp_real) :: alpha_rad, a, b
  
    alpha_rad=alpha*(4.0_dp_real*ATAN(1.0_dp_real)/180.0) ! deg to radian
  ...
  END SUBROUTINE rotation_about_z

SUBROUTINE rotation_about_u(alpha, uvec, rot_mat)
IMPLICIT NONE 
REAL (dp_real), INTENT(IN) :: alpha, uvec(3) 
REAL (dp_real), INTENT(OUT) :: rot_mat(3,3)

REAL (dp_real) :: alpha_rad, a, b, ux, uy, uz 
  ...
  rot_mat(1,1) = a + (ux**2*(1.0-a))
  rot_mat(1,2) = (ux*uy*(1.0-a))- (uz*b) 
  rot_mat(1,3) = (ux*uz*(1.0-a)) + (uy*b)

  rot_mat(2,1) = (uy*ux*(1.0-a)) + (uz*b)
  rot_mat(2,2) = a + (uy**2*(1.0-a))
  rot_mat(2,3) = (uy*uz*(1.0-a))-(ux*b)

  rot_mat(3,1) = (uz*ux*(1.0-a))-(uy*b)
  rot_mat(3,2) = (uz*uy*(1.0-a)) + (ux*b)
  rot_mat(3,3) = a + (uz**2*(1.0-a))

END SUBROUTINE rotation_about_u
    ...
    DO iz =0, nz-1
        DO iy = 0, ny-1
           ...
           elseif (flag==2)then
                 ...
                 a_atms_opt(1,n) = DBLE(iz)
                 b_atms_opt(1,n) = DBLE(iz)
                o1_atms_opt(1,n) = DBLE(iz)
                o2_atms_opt(1,n) = DBLE(iz)
                o3_atms_opt(1,n) = DBLE(iz)
...
SUBROUTINE ab_1st_neighbours(n1st, dmin, n1, n2, atm1, atm2, neigh_ab)
IMPLICIT NONE
INTEGER(dp_int), INTENT(IN) :: n1st, n1, n2  ! 
REAL (dp_real) :: dmin
REAL(dp_real), dimension(ndim4,n1), INTENT(IN) ::  atm1
REAL(dp_real), dimension(ndim4,n2), INTENT(IN) ::  atm2
INTEGER(dp_int), INTENT(OUT) :: neigh_ab(n1st,n1)
...
END SUBROUTINE ab_1st_neighbours
...
SUBROUTINE dm_Zint_angle(nsti, latt_vec, b1_atms, o1_atms, angle1_ti_o_ti)
        
  IMPLICIT NONE

  INTEGER(dp_int), INTENT(IN) :: nsti
  INTEGER(dp_int), INTENT(INOUT) :: angle1_ti_o_ti(3,nsti)
  REAL(dp_real), INTENT(IN) :: b1_atms(4,nsti), o1_atms(4,nsti), latt_vec(3,3)


  REAL(dp_real) :: r1(3), r2(3), v1(3), v2(3), vo(3), r1_norm, r2_norm

   !REAL(dp_real) :: d_feofe(2)
 ...
 OPEN(UNIT=71, FILE='angle_info_zaxis.dat', ACTION='WRITE')
  ...
  DO j =1, nsti
     ...
     IF (r_dot+1 <= 0.001 .AND. (r2_norm-dmin) <0.0) THEN 
        ...
        ncount = ncount + 1
        IF (NINT(r_dot) /=-1) PRINT*, "----WRONG---", r_dot !, d_feofe(1), d_feofe(2) 
     ENDIF
      ...
      DO j =1, nsti
       ...
       DO iz = -1, 1, 2
        ...
        IF ((r2_norm-dmin) <0) THEN
            ...
            ncount = ncount + 1
            IF (NINT(DOT_PRODUCT(r1,r2)) /=-1) PRINT*, "----WRONG--- between Fe-O-Fe", r_dot
...
SUBROUTINE projection_car_axis(vec_unit, nsti, atm1, atm2, neigh_ab, NeighAlongAxis)
   ...
   INTEGER(dp_int), INTENT(IN) :: nsti, neigh_ab(8, nsti)
        ...
        REAL(dp_real), INTENT(IN) :: atm1(4, nsti), atm2(4, nsti), vec_unit(3)
...
SUBROUTINE projection_car_xy1(nsti, atm1, atm2, neigh_ab, NeighAlongAxis)
IMPLICIT NONE

INTEGER(dp_int), INTENT(IN) :: nsti, neigh_ab(8, nsti)

  REAL(dp_real), INTENT(IN) :: atm1(4, nsti), atm2(4, nsti)
...
END SUBROUTINE projection_car_xy1
...
SUBROUTINE dm_int_angle(nsti, latt_vec, b1_atms, o11_atms, angle1_ti_o_ti)
IMPLICIT NONE
INTEGER(dp_int), INTENT(IN) :: nsti
INTEGER(dp_int), INTENT(INOUT) :: angle1_ti_o_ti(3, nsti)
REAL(dp_real), INTENT(IN) :: b1_atms(4, nsti), o11_atms(4, nsti), latt_vec(3,3)
...
ncount = ncount + 1
IF (NINT(r_dot) /=-1) PRINT*, "----WRONG---", r_dot
ENDIF 
 ...
 DO ix = -1, 1
  ...
  IF ((r2_norm-dmin) <0) THEN
     ...
     ! d_feofe(2) = r2_norm   
      ...
      ncount = ncount + 1
     IF (NINT(DOT_PRODUCT(r1,r2)) /=-1) PRINT*, "----WRONG--- (dm_int_angle)", DOT_PRODUCT(r1,r2) !,  d_feofe(1),  d_feofe(2)
  ENDIF
