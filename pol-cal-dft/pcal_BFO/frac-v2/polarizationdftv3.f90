...
PROGRAM polarization_cal
 ...
 REAL(DP), PARAMETER  :: qpb=4.92d0, qti=4.25d0, qmg=2.0, qnb=7.5, &
...
!no_neigh=6
!Call displacement(no_neigh, nmg, no, a, mg_atm(1:nmg,:), o_atm(1:no,:),
!mg_disp)
...
!   ENDDO
!WRITE(18,200) i, (mg_disp(i,j), j=1,3), SQRT(mg_disp(i,1)**2 + mg_disp(i,2)**2
!+ mg_disp(i,3)**2)
!        ENDDO
!!
!!! Calculating displacement for B prime atoms
!no_neigh=6
!Call displacement(no_neigh, nnb, no, a, nb_atm(1:nnb,:), o_atm(1:no,:),
!nb_disp)
!!! Polarization due to displacement of Pb atom relative to 6 O atoms
!p_nb=(/0.0,0.0,0.0/)
!DO i=1, nnb
...
!
!   ENDDO
!WRITE(19,200) i, (nb_disp(i,j), j=1,3), SQRT(nb_disp(i,1)**2 + nb_disp(i,2)**2
!+ nb_disp(i,3)**2)
!        ENDDO
!!
...
!CALL  nearest_neighbour(no_neigh, nmg, npb, a, mg_atm(1:nmg,:),
!pb_atm(1:npb,:), mg_pb)
!CALL  nearest_neighbour(no_neigh, nnb, npb, a, nb_atm(1:nnb,:),
!pb_atm(1:npb,:), nb_pb)
!!DO i=1, nb
!!  PRINT*, (ab(i,j),j=1,8)
!!ENDDO
...
WRITE(14,*) ntot, '1'
DO i=1, npb
f(1)=pb_disp(i,1)*qpb/unitvol*16.02
f(2)=pb_disp(i,2)*qpb/unitvol*16.02
f(3)=pb_disp(i,3)*qpb/unitvol*16.02
...
f(1)=ti_disp(i,1)*qti/unitvol*16.02
f(2)=ti_disp(i,2)*qti/unitvol*16.02
f(3)=ti_disp(i,3)*qti/unitvol*16.02

WRITE(14,301) atmnum(2), (ti_atm(i,j),j=1,3), f(1), f(2), f(3) !(punit(j),j=1,3)
ENDDO
...
!    ENDDO
!!    PRINT*, ptemp/8.0, pb(j), pa(j)
! punit(j)=(mg_disp(i,j)*qmg + (ptemp/8.0))/unitvol*16.02  ! Total polarization
! per unit cell
!ENDDO
...
!    ENDDO
!!!    PRINT*, ptemp/8.0, pb(j), pa(j)
! punit(j)=(nb_disp(i,j)*qnb + (ptemp/8.0))/unitvol*16.02  ! Total polarization
! per unit cell
!ENDDO
